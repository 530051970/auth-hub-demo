    ...
    commands:
      ...
      - |
        ...
        if [ $WAIT_STATUS -eq 0 ]; then
          ...
          reason_detail=$(aws cloudformation describe-stack-events --stack-name intelli-agent | jq '[.StackEvents[] | select(.ResourceType=="AWS::EC2::Subnet" and .ResourceStatus=="DELETE_FAILED")] | last')
      ...
      - |
        ...
        if [ -d "Intelli-Agent/source/infrastructure" ]; then
          ...
          if [ ${PIPESTATUS[0]} -ne 0 ]; then
            ...
            jq -n --arg error "$(cat synth.log)" '{error: $error}' > sync_error.json
            aws lambda invoke --function-name $GEN_REPORT_FUNCTION --payload fileb://payload.json sync_error.json
            exit 1
          else
        ...
        if [ "$DEPLOY_STACK" = "cdk" ]; then
          if [ -d "Intelli-Agent/source/infrastructure" ]; then
            pushd "Intelli-Agent/source/lambda/job"
            echo build_whl.sh start
            chmod +x build_whl.sh
            ./build_whl.sh
            echo build_whl.sh end
            popd
            pushd "Intelli-Agent/source/infrastructure"
  ...
  build:
    commands:
      - |
        ...
        pytest test_case --continue-on-collection-errors --log-cli-level=INFO --json-report --json-report-file=detail.json --html=report.html --self-contained-html -log-cli=true > detail.log
        popd
